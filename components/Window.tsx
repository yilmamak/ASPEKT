...
export function Window({ title = 'ASPEKT', leftSlot, rightSlot, children, className = '' }: WindowProps) {
  return (
    <div className={`relative bg-[#0f0f0f]/90 border border-[#1f1f1f] rounded-xl overflow-hidden shadow-[0_24px_80px_-16px_rgba(0,0,0,0.8),0_20px_80px_-20px_rgba(94,106,210,0.18)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_32px_96px_-16px_rgba(0,0,0,0.9),0_20px_100px_-10px_rgba(94,106,210,0.28)] ${className}`}>
      {/* Ambient glow */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#5E6AD2]/10 via-transparent to-transparent pointer-events-none z-0" />
      ...
      <div className="relative z-10 flex items-center px-3.5 py-2.5 bg-[#0c0c0c] border-b border-[#1f1f1f] gap-2">
        ...
        <div className="flex-1 text-center text-[12px] font-medium text-[#f0f0f0] font-sans">{title}</div>
